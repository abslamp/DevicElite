...
public class UpdateRepairRecordTaskServlet extends HttpServlet {
...
//			}
//			
//			RepairRecord rr = new RepairRecord(b, c, null, "","", null, "",d,e,"","","","", a);
			
			RepairRecord rr = new RepairRecord();
			rr.init();
			if(!repair_record_no.equals("")){
				rr.setRepair_record_no(Integer.parseInt(repair_record_no));
			}else{
				System.out.println("UpdateRepairRecordTaskServlet:Empty repair_record_no.");
			}
			rr.setRepair_status("已分配");
			if(!repair_person_no.equals("")){
				rr.setRepair_person_no(Integer.parseInt(repair_person_no));
			}
...
//			out.println("setTimeout(function {document.forms[0].submit();},1000);"); 
			out.println("self.location='/DevicElite/Task/QueryRepairRecord.jsp';");
			out.println("</script>");

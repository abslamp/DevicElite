...
public class AddCustomerServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Date time = DateUtil.HTMLtoServlet(customer_visittime);
		...
		boolean result = dao.addCustomer(cus);
		...
		if(result)	
		{
			...
			request.getRequestDispatcher("/Service/addCustomer.jsp").forward(request, response);
		}
		else
		{
			PrintWriter out = response.getWriter();
			out.println("<script>");
			out.println("alert('"+(result?"添加成功!":"添加失败!")+"');");
